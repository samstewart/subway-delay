...
one_route = pd.DataFrame({ 'stop_name': stations})
one_route['route'] = '1'
one_route['direction'] = 1 
one_route['stop_index'] = range(len(stations))
one_route.to_csv('data/raw/static_transit/route_stops.csv', sep=',')
one_route
plt.cla()
plt.plot(times, stations, linestyle='--')
for i in longest.index:
